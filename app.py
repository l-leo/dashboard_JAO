...
def value_to_heat_color(value: float, min_value: float, max_value: float) -> str:
...
def render_hourly_heatmap(values: pd.Series, title: str) -> None:
    ...
    header_cells = "".join([f"<th style='padding:4px;font-size:12px'>{hour:02d}</th>" for hour in range(24)])
    value_cells = ""
    for hour in range(24):
        value = float(values.iloc[hour])
        color = value_to_heat_color(value, min_value, max_value)
        text_color = "#ffffff" if value > (min_value + max_value) / 2 else "#111111"
        value_cells += (
            f"<td style='padding:6px;text-align:center;background:{color};color:{text_color};font-size:12px'>"
            f"{value:.2f}</td>"
        )
    html = (
        f"<div style='margin-bottom:6px;font-weight:600'>{title}</div>"
